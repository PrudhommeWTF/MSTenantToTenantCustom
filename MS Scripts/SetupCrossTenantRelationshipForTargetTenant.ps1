...
function Main() {
    ...
    if ($PSCmdlet.ParameterSetName -eq 'TargetSetupAll' -or $PSCmdlet.ParameterSetName -eq 'TargetSetupAzure') {
        ...
        $subscription = $subscriptions | Where-Objecthere-Object { $_.SubscriptionId -eq $SubscriptionId}
        if (-not $subscription) {
        ...
        $exoAppSpn = $spns | Where-Object { $_.AppId -eq $EXO_APP_ID }
...
function Import-AzureModules() {
    ...
    $moduleMissingErrors = @()
    $desiredAzureModules.Keys | ForEach-Object {
        $desiredVersion = [Version]($desiredAzureModules[$_])
        $desiredAzureRmModule = (Get-Module $_ -ListAvailable -Verbose:$false | Where-Object { $_.Version -ge $desiredVersion})
...
function Create-Application([string]$targetTenantDomain, [string]$resourceTenantDomain, $certificate, $spns, $azAppPermissions, [guid]$ExistingApplicationId) {
    ...
    if ($azAppPermissions.HasFlag([ApplicationPermissions]::MSGraph)) {
        ...
        $msGraphSpn = $spns | Where-Object { $_.AppId -eq $MS_GRAPH_APP_ID }
        if (-not $msGraphSpn) {
    ...
    foreach ($role in $MS_GRAPH_APP_ROLE) {
            $reqGraph.ResourceAccess += New-Object -TypeName "Microsoft.Open.AzureAD.Model.ResourceAccess" -ArgumentList ($msGraphSpn.AppRoles | Where-Object { $_.Value -eq $role }).Id,"Role"
        }
    }
    ...
    if ($azAppPermissions.HasFlag([ApplicationPermissions]::Exchange)) {
        ...
        $exoAppPermission = $exoAppSpn.AppRoles | Where-Object { $_.Value -eq $EXO_APP_ROLE }
...
function Run-ExchangeSetupForTargetTenant([string]$targetTenant, [string]$resourceTenantDomain, [string]$resourceTenantId, [string]$appId, [string]$appSecretKeyVaultUrl) {
    ...
    $orgRel = Get-OrganizationRelationship | Where-Object { $_.DomainNames -contains $resourceTenantId }
    if ($orgRel) {
